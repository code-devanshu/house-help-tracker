...
import { syncAppData } from "../action";
...
export default function WorkerTrackerPage() {
  ...
  const sync = () => {
    startTransition(async () => {
      const result = await syncAppData(loadAppData());
      if (!result.ok) showToast("Sync failed. Changes saved locally.", "error");
    });
  };

  const [lastUpdatedISO, setLastUpdatedISO] = useState<string | null>(null);
  const lastUpdatedTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    ...
    if (store) { setEntries(store.entries.filter((e) => e.workerId === workerId)); setAutoFillInfo(yesterdayISO); sync(); }
  ...
  const upsertWithStatus = (iso: string, status: ShiftStatus) => {
    ...
    if (lastUpdatedTimer.current) clearTimeout(lastUpdatedTimer.current);
    lastUpdatedTimer.current = setTimeout(() => setLastUpdatedISO(null), 1400);
    sync();
  };
  ...
  return (
    <main className="px-4 pt-6 pb-20 text-slate-900 dark:text-white">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <Link
          href="/workers"
          className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-sm text-slate-600 transition hover:bg-slate-200 hover:text-slate-900 dark:border-white/8 dark:bg-white/3 dark:text-white/60 dark:hover:bg-white/6 dark:hover:text-white/80"
        >
      ...
      {/* ── Worker header card ── */}
      <div className="mb-6 rounded-2xl border border-slate-200 bg-slate-50 px-5 py-5 dark:border-white/7 dark:bg-white/3">
        <div className="flex flex-wrap items-start justify-between gap-4">
          {/* Identity */}
          <div className="flex items-center gap-4">
            <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-linear-to-br ${avatarColors[hue]} ring-1 ring-black/5 dark:ring-white/10`}>
              <span className="text-sm font-bold text-white">{workerInitials}</span>
            </div>
            <div>
              <h1 className="text-xl font-semibold tracking-tight text-slate-900 dark:text-white">{worker.name}</h1>
              <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-500 dark:text-white/45">
                ...
                {editingStartDate ? (
                  <span className="flex items-center gap-1.5">
                    <input
                      ...
                      className="h-6 rounded-lg border border-indigo-400/40 bg-slate-50 px-2 text-xs text-slate-900 outline-none focus:ring-2 focus:ring-indigo-500/20 dark:bg-white/6 dark:text-white dark:scheme-dark"
                    />
                    <button onClick={saveStartDate} className="text-indigo-600 transition text-[11px] font-semibold hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300">Save</button>
                    <button onClick={() => setEditingStartDate(false)} className="text-slate-400 transition text-[11px] hover:text-slate-600 dark:text-white/30 dark:hover:text-white/60">Cancel</button>
                  </span>
                ) : (
                  <button
                    onClick={() => { setStartDateDraft(worker.startDate ?? ""); setEditingStartDate(true); }}
                    className="flex items-center gap-1 text-slate-500 transition hover:text-slate-700 dark:text-white/45 dark:hover:text-white/70"
                  >
                    <svg className="h-3 w-3 text-slate-400 dark:text-white/30" viewBox="0 0 12 12" fill="none">
                    ...
                    {worker.startDate
                      ...
                      : <span className="text-slate-400 dark:text-white/30">Set start date</span>}
                  </button>
                )}
              </div>
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Lock toggle */}
            <div className="flex items-center gap-2.5 rounded-xl border border-slate-200 bg-white px-3 py-2 dark:border-white/7 dark:bg-white/3">
              <span className={`text-xs font-medium ${isMonthLocked ? "text-amber-500 dark:text-amber-400" : "text-slate-500 dark:text-white/45"}`}>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          {isSyncing && (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-500 dark:bg-white/6 dark:text-white/40">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white dark:border-white/7 dark:bg-white/2.5">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-white/7">
            <div className="text-sm font-semibold text-slate-800 dark:text-white/85">Attendance</div>
            {/* Legend */}
          ...
          <div className="p-4 sm:p-5">
            ...
            <div className="mb-2 grid grid-cols-7 gap-1.5">
              {weekdayShort.map((w) => (
                <div key={w} className="py-1 text-center text-[11px] font-semibold text-slate-400 dark:text-white/30">{w}</div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-7 gap-1.5">
              ...
              {monthDays.map((d) => {
                ...
                const justUpdated = lastUpdatedISO === iso;
                ...
                return (
                  <button
                    ...
                    className={[
                      "relative flex min-h-11 flex-col items-center justify-center rounded-xl border pt-1 pb-1 text-center transition-all duration-200",
                      isBeforeStart ? "cursor-not-allowed border-slate-100 bg-transparent opacity-25 dark:border-white/3" :
                      disabled ? "cursor-not-allowed opacity-30" : "cursor-pointer hover:border-slate-300 hover:bg-slate-50 dark:hover:border-white/20 dark:hover:bg-white/5",
                      !isBeforeStart && (justUpdated ? "border-emerald-400/60 bg-emerald-500/10 ring-2 ring-emerald-500/20 dark:bg-emerald-500/10" :
                      isSelected ? "border-indigo-400/60 bg-indigo-500/10 ring-2 ring-indigo-500/20" :
                      isToday ? "border-indigo-300/40 bg-indigo-500/8" : "border-slate-200 bg-slate-50 dark:border-white/6 dark:bg-white/2"),
                    ].join(" ")}
                  >
                    {isToday && <div className="absolute top-0 left-1/2 h-0.5 w-5 -translate-x-1/2 rounded-b-full bg-indigo-400" />}

                    <span className={`text-xs font-semibold ${isToday ? "text-indigo-500 dark:text-indigo-300" : "text-slate-600 dark:text-white/75"}`}>
                      {d.getDate()}
                    </span>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-white/7 dark:bg-white/2.5">
            <div className="mb-4 text-sm font-semibold text-slate-800 dark:text-white/85">Summary</div>
            <div className="grid grid-cols-2 gap-2.5">
              {[
                { label: "Worked", value: totals.worked, color: "text-emerald-600 dark:text-emerald-400" },
                { label: "Absent", value: totals.absent, color: "text-rose-500 dark:text-rose-400" },
                { label: "Half", value: totals.half, color: "text-amber-600 dark:text-amber-400" },
                { label: "Off", value: totals.off, color: "text-slate-500 dark:text-slate-400" },
              ].map((x) => (
                <div key={x.label} className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-white/6 dark:bg-white/2.5">
                  <div className="text-[11px] font-medium text-slate-500 dark:text-white/40">{x.label}</div>
                  <div className={`mt-1 text-2xl font-bold ${x.color}`}>{x.value}</div>
                </div>
              ))}
            </div>
            {totals.hours > 0 && (
              <div className="mt-2.5 rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-white/6 dark:bg-white/2.5">
                <div className="text-[11px] font-medium text-slate-500 dark:text-white/40">Total hours</div>
                <div className="mt-1 text-2xl font-bold text-slate-700 dark:text-white/80">{totals.hours}h</div>
              </div>
            )}
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-white/7 dark:bg-white/2.5">
            <div className="mb-4 flex items-center justify-between">
              <div className="text-sm font-semibold text-slate-800 dark:text-white/85">Day Details</div>
              {selectedISO && <div className="text-xs text-slate-400 dark:text-white/35">{selectedISO}</div>}
            </div>

            {!selectedISO ? (
              <div className="flex flex-col items-center gap-2 py-6 text-center">
                <div className="text-2xl opacity-30">📅</div>
                <div className="text-xs text-slate-400 dark:text-white/30">Tap any past day to view or edit details</div>
              </div>
            ) : !selectedEntry ? (
              <div className="text-xs text-slate-400 dark:text-white/35">No entry recorded.</div>
            ) : (
              <div className="space-y-4">
                ...
                <label className="grid gap-1.5">
                  <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-white/35">Hours <span className="normal-case font-normal tracking-normal">(optional)</span></span>
                  <input
                    ...
                    className="h-10 rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm text-slate-900 outline-none transition focus:border-indigo-400/50 focus:ring-4 focus:ring-indigo-500/10 disabled:opacity-40 dark:border-white/10 dark:bg-white/4 dark:text-white"
                    placeholder="e.g. 8"
                  />
                </label>

                <label className="grid gap-1.5">
                  <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-white/35">Note <span className="normal-case font-normal tracking-normal">(optional)</span></span>
                  <textarea
                    disabled={isMonthLocked}
                    value={selectedEntry.note ?? ""}
                    onChange={(e) => updateSelected({ note: e.target.value })}
                    className="min-h-20 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 outline-none transition focus:border-indigo-400/50 focus:ring-4 focus:ring-indigo-500/10 disabled:opacity-40 resize-none dark:border-white/10 dark:bg-white/4 dark:text-white"
                    placeholder="Late arrival, extra cleaning…"
                  />
                </label>

                {isMonthLocked && <div className="text-xs text-amber-500 dark:text-amber-400/60">Month is locked. Unlock to edit.</div>}
              </div>
            )}
          </div>
        </div>
      ...
      {picker.open && picker.iso && (
        <div
          ref={popoverRef}
          className="fixed z-50 w-56 rounded-2xl border border-slate-200 bg-white shadow-[0_20px_60px_rgba(0,0,0,0.15)] dark:border-white/10 dark:bg-[#0D1117] dark:shadow-[0_20px_60px_rgba(0,0,0,0.7)]"
          style={{ left: picker.x, top: picker.y }}
          onPointerDown={(e) => e.stopPropagation()}
          onMouseDown={(e) => e.stopPropagation()}
        >
          <div className="px-3 pt-3 pb-1 text-[11px] font-semibold text-slate-500 dark:text-white/40">{picker.iso}</div>
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-sm transition ${
                    active ? "bg-slate-100 text-slate-900 dark:bg-white/10 dark:text-white" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900 dark:text-white/70 dark:hover:bg-white/6 dark:hover:text-white"
                  }`}
                >
                  ...
                  {active && <span className="text-xs text-indigo-500 dark:text-indigo-400">✓</span>}
                </button>
              );
            })}
          </div>
          <div className="border-t border-slate-100 px-3 py-2 text-[10px] text-slate-400 dark:border-white/7 dark:text-white/25">Press Esc to close</div>
        </div>
      )}
      ...
      {lockConfirmOpen && (
        ...
        >
          ...
          <div className="relative w-full max-w-sm rounded-2xl border border-slate-200 bg-white shadow-[0_24px_64px_rgba(0,0,0,0.15)] dark:border-white/10 dark:bg-[#0D1117] dark:shadow-[0_24px_64px_rgba(0,0,0,0.7)]">
            <div className="border-b border-slate-100 px-5 py-4 dark:border-white/7">
              <div className="text-sm font-semibold text-slate-900 dark:text-white">Lock {monthKey}?</div>
              <div className="mt-0.5 text-xs text-slate-500 dark:text-white/40">No edits will be possible until you unlock.</div>
            </div>
            <div className="px-5 py-5 space-y-4">
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/8 px-4 py-3 text-xs text-amber-700 dark:text-amber-200/80">
                Attendance, salary settings, and deductions will all be read-only.
              </div>
              <div className="flex gap-3">
                <button onClick={() => setLockConfirmOpen(false)} className="h-10 flex-1 rounded-xl border border-slate-200 bg-slate-50 text-sm font-medium text-slate-600 transition hover:bg-slate-100 dark:border-white/10 dark:bg-white/4 dark:text-white/60 dark:hover:bg-white/7">
                  Cancel
                </button>
                <button
                  onClick={() => { setLocked(true); setLockConfirmOpen(false); }}
                  className="h-10 flex-1 rounded-xl bg-amber-500 text-sm font-semibold text-white transition hover:bg-amber-400"
                >
                  Lock month
                </button>
              </div>
