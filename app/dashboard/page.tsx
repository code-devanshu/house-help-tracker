...
function ChartTooltip({
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-xl dark:border-white/10 dark:bg-slate-900">
      <div className="mb-0.5 text-xs text-slate-500 dark:text-white/45">{label}</div>
      <div className="text-sm font-bold text-emerald-600 dark:text-emerald-300">₹{money(payload[0]!.value)}</div>
    </div>
  );
}
  ...
  if (initialLoading) {
    return (
      <main className="px-4 pt-8 pb-16">
        <div className="mb-8 h-8 w-48 animate-pulse rounded-lg bg-slate-200 dark:bg-white/6" />
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-24 animate-pulse rounded-2xl border border-slate-200 bg-slate-100 dark:border-white/6 dark:bg-white/4" />
          ))}
        </div>
        <div className="mt-6 h-56 animate-pulse rounded-2xl border border-slate-200 bg-slate-100 dark:border-white/6 dark:bg-white/4" />
        <div className="mt-6 h-48 animate-pulse rounded-2xl border border-slate-200 bg-slate-100 dark:border-white/6 dark:bg-white/4" />
      </main>
    );
  }
  ...
  if (allWorkers.length === 0) {
    return (
      <main className="flex min-h-[60vh] flex-col items-center justify-center gap-4 px-4 text-center">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-100 text-3xl dark:bg-white/5">📊</div>
        <div>
          <div className="text-base font-semibold text-slate-700 dark:text-white/70">No data yet</div>
          <div className="mt-1 text-sm text-slate-400 dark:text-white/35">Add workers and mark attendance to see your dashboard.</div>
        </div>
        <Link
          href="/workers"
    ...
    <main className="px-4 pt-8 pb-16">
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500 dark:text-white/40">
            ...
            {allWorkers.length > activeWorkers.length && (
              <span className="text-slate-300 dark:text-white/25"> · {allWorkers.length - activeWorkers.length} archived</span>
            )}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <Link
            href="/workers"
            className="inline-flex h-9 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 text-sm font-semibold text-slate-600 transition hover:bg-slate-100 hover:text-slate-900 dark:border-white/10 dark:bg-white/5 dark:text-white/70 dark:hover:bg-white/10 dark:hover:text-white"
          >
            Manage Workers
      ...
      {/* ── Monthly trend ── */}
      <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.06)] dark:border-white/7 dark:bg-white/2.5 dark:shadow-[0_1px_3px_rgba(0,0,0,0.3)]">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <div className="text-sm font-semibold text-slate-700 dark:text-white/80">Monthly spend</div>
            <div className="mt-0.5 text-xs text-slate-400 dark:text-white/35">Last 6 months · all workers</div>
          </div>
        </div>
        ...
        {mounted && (
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={trendData} barSize={32} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
              <XAxis
                dataKey="label"
                tick={{ fill: isDark ? "rgba(255,255,255,0.35)" : "rgba(71,85,105,0.8)", fontSize: 12 }}
                axisLine={false}
                tickLine={false}
              />
              <YAxis
                tick={{ fill: isDark ? "rgba(255,255,255,0.25)" : "rgba(100,116,139,0.7)", fontSize: 11 }}
                axisLine={false}
                tickLine={false}
                  ...
                  <Cell
                    ...
                    fill={
                      entry.monthKey === mkKey(currentMonthStart)
                        ...
                        : isDark ? "rgba(99,102,241,0.35)" : "rgba(99,102,241,0.5)"
                    }
                  />
                ))}
              </Bar>
      ...
      <section className="mt-5 rounded-2xl border border-slate-200 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)] dark:border-white/7 dark:bg-white/2.5 dark:shadow-[0_1px_3px_rgba(0,0,0,0.3)]">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-white/6">
          <div>
            <div className="text-sm font-semibold text-slate-700 dark:text-white/80">Worker breakdown</div>
            <div className="mt-0.5 text-xs text-slate-400 dark:text-white/35">{monthLabel(selectedMonth)}</div>
          </div>
        ...
        {workerBreakdown.length === 0 ? (
          <div className="flex flex-col items-center gap-2 px-6 py-10 text-center">
            ...
            <div className="text-sm text-slate-400 dark:text-white/40">No workers had started by this month</div>
          </div>
        ) : (
...
function StatCard({
  ...
  const accentCls = {
    emerald: { card: "from-emerald-500/10 to-emerald-500/5 border-emerald-500/20", value: "text-emerald-600 dark:text-emerald-300" },
    indigo:  { card: "from-indigo-500/10 to-indigo-500/5 border-indigo-500/20",   value: "text-indigo-600 dark:text-indigo-300" },
    violet:  { card: "from-violet-500/10 to-violet-500/5 border-violet-500/20",   value: "text-violet-600 dark:text-violet-300" },
  }[accent];
  return (
    <div
      className={`rounded-2xl border bg-linear-to-br px-5 py-4 shadow-[0_1px_3px_rgba(0,0,0,0.06)] dark:shadow-[0_1px_3px_rgba(0,0,0,0.3)] ${accentCls.card}`}
    >
      <div className="text-xs text-slate-500 dark:text-white/40">{label}</div>
      <div className={`mt-2 text-2xl font-bold ${accentCls.value}`}>{value}</div>
      <div className="mt-1 text-xs text-slate-400 dark:text-white/30">{sub}</div>
    </div>
  );
}
